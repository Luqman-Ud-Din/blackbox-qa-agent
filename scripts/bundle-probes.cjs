...
window.__ARGUS_PROBES = {
  skills: [\n${entries}\n],
  // Viewport gate. (1) applyOn must include this viewport class. (2) LEADER GATE: a
  // viewportSensitive:false skill finds the SAME bug at every width, so it runs ONCE — only
  // on the run's leader viewport (the first of mobile→tablet→laptop→desktop present this run,
  // passed in as leaderVp). vs:true skills are width-dependent and run on every viewport.
  // This is what makes the "run once" flag actually reduce execution (previously it was read
  // only by coverage-gate, so invariant skills still ran + emitted on all 4 viewports).
  _applies: function(s, vp, leaderVp){
    var ok = s.applyOn === 'all' || (Array.isArray(s.applyOn) && s.applyOn.indexOf(vp) !== -1);
    if (!ok) return false;
    if (s.viewportSensitive === false && leaderVp && vp !== leaderVp) return false;
    return true;
  },
